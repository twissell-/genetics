...
def newPopulation(popSize, gens, ran=2):
    ...
    for x in range(0, popSize):
        ...
        for z in range(gens):
            dic.append(z)
        while True:
            n = dic.pop(dic.index(choice(dic)))
...
def getDistance(city, otherCity):
    '''Returns distances between city and otherCity It require the data
    package'''
...
def chrToString(chr):
    '''Receive a chromesome (a list of int) and return a string
    representation'''
...
#    CODIGO A PARTIR DE INGENIERIA INVERSA.
#    Original en javascript por @juanipaccapelo

#    ofValues = objetiveFunc(population)
#    ffValues = []
#    auxObj = []
#    for x in ofValues:
#        auxObj.append(sum(ofValues) - x)
#    for i in range(len(ofValues)):
#        ffValues.append(auxObj[i] / sum(auxObj))
#    return ffValues
...
#        Inicialize a new son
    ...
    chrLen = len(population[0])  # lint:ok
    nextPopulation = []

    for x in filter(lambda n : n % 2 != 0, range(len(population))):  # lint:ok
